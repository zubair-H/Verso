...
const HAIR_COLOR_PRESETS = [
  ...
  { id: 'dark_brown', name: 'Dark Brown', hex: '#2a1b12', strength: 0.70 },
  { id: 'light_brown', name: 'Light Brown', hex: '#6b4a2f', strength: 0.65 },
  { id: 'blonde', name: 'Blonde', hex: '#d8c07a', strength: 0.60 },
...
async function recolorHairWithMask({ imageBuf, maskBuf, hex, strength = 0.6 }) {
  // Ensure both are same size
  const img = sharp(imageBuf).ensureAlpha();
  const meta = await img.metadata();
  const w = meta.width;
  const h = meta.height;
  ...
  const mask = sharp(maskBuf)
    ...
    .ensureAlpha()
    .grayscale()
    .threshold(140)      // hair = white
    .blur(1.2);          // soften edges
  ...
  // Blend color into image (keeps texture)
  // We do a gentle "tint": original + (color * strength) using overlay-ish blend,
  // then apply ONLY where mask is white using 'dest-in'.
  const tinted = await img
    .composite([{ input: colorLayer, blend: 'overlay', opacity: strength }])
    .png()
    .toBuffer();

  const maskPng = await mask.png().toBuffer();

  // Apply mask to tinted layer (keep only hair region)
  const tintedHairOnly = await sharp(tinted)
    .composite([{ input: maskPng, blend: 'dest-in' }])
    .png()
    .toBuffer();

  // Put tinted hair back over original
  const out = await img
    .composite([{ input: tintedHairOnly, blend: 'over' }])
    .png()
    .toBuffer();
...
if (method === 'GET' && pathname === '/api/hair-colors') {
  return sendJson(res, 200, {
    success: true,
    colors: HAIR_COLOR_PRESETS.map(({ id, name, hex }) => ({ id, name, hex })),
  });
}
...
if (method === 'POST' && pathname === '/api/recolor-hair') {
  try {
    ...
    const preset = colorId ? HAIR_COLOR_PRESETS.find(c => c.id === colorId) : null;
    const targetHex = (hex || preset?.hex || '').trim();
    const targetStrength = typeof strength === 'number' ? strength : (preset?.strength ?? 0.6);

    if (!targetHex) return sendJson(res, 400, { error: 'Provide colorId or hex' });

    // Convert user image URL -> data URI buffer for consistent ingest
    const userImageData = userImageUrl.startsWith('data:') ? userImageUrl : await urlToDataUri(userImageUrl);

    // 1) Hair mask via Replicate (still AI, but only segmentation)
    const rawMaskUrl = await generateHairMask(userImageData);

    // 2) Fetch buffers
    const imageBuf = await fetchBuffer(userImageData);
    const maskBuf = await fetchBuffer(rawMaskUrl);

    // 3) Recolor locally (fast + stable)
    const outBuf = await recolorHairWithMask({
      imageBuf,
      maskBuf,
      hex: targetHex,
    ...
    // Return as data URI (simple for now)
    const outDataUri = `data:image/png;base64,${outBuf.toString('base64')}`;

    return sendJson(res, 200, {
      success: true,
      color: { id: preset?.id || null, name: preset?.name || null, hex: targetHex, strength: targetStrength },
      maskUrl: rawMaskUrl,
      editedImageDataUri: outDataUri,
    });
  } catch (err) {
    console.error('Recolor error:', err);
    return sendJson(res, 500, { error: err.message || 'Recolor failed' });
  }
}
