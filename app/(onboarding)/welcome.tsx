...
// Floating bubble component with entry + exit animation support
function FloatingBubble({
  size, x, y, color, opacity, duration, delay, entryProgress, exitProgress
}: {
...
export default function WelcomeScreen() {
  ...
  const bubbleEntryProgress = useSharedValue(0);
  ...
  useEffect(() => {
    // No logo delay - content appears quickly since logo already animated on previous screen

    // Bubbles scale in elegantly
    bubbleEntryProgress.value = withTiming(1, { duration: 800, easing: SMOOTH_EASE });

    // Tagline line 1
    tagline1Opacity.value = withDelay(200, withTiming(1, { duration: 600, easing: SMOOTH_EASE }));
    tagline1TranslateY.value = withDelay(200, withSpring(0, { damping: 22, stiffness: 85 }));

    // Tagline line 2
    tagline2Opacity.value = withDelay(350, withTiming(1, { duration: 600, easing: SMOOTH_EASE }));
    tagline2TranslateY.value = withDelay(350, withSpring(0, { damping: 22, stiffness: 85 }));

    // Subtitle
    subtitleOpacity.value = withDelay(550, withTiming(1, { duration: 650, easing: SMOOTH_EASE }));
    subtitleTranslateY.value = withDelay(550, withSpring(0, { damping: 24, stiffness: 80 }));

    // Button
    buttonOpacity.value = withDelay(700, withTiming(1, { duration: 500, easing: SMOOTH_EASE }));
    buttonTranslateY.value = withDelay(700, withSpring(0, { damping: 20, stiffness: 90 }));
  }, []);
      ...
      <View style={styles.bubblesLayer}>
        {BUBBLES.map((bubble, index) => (
          <FloatingBubble
            key={index}
            {...bubble}
            entryProgress={bubbleEntryProgress}
